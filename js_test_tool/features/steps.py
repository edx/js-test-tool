...
EXPECTED_COVERAGE_XML = 'expected_js_coverage.xml'
EXPECTED_COVERAGE_HTML = 'expected_js_coverage.html'
EXPECTED_TEST_REPORT = 'expected_test_report.txt'
...
def check_xml_coverage_report(step):
    world.compare_files_at_path(ACTUAL_COVERAGE_XML, EXPECTED_COVERAGE_XML)


@step(u'Then An HTML coverage report is generated')
def check_html_coverage_report(step):
    world.compare_files_at_path(ACTUAL_COVERAGE_HTML, EXPECTED_COVERAGE_HTML)


@step(u'Then No coverage reports are generated')
def check_no_coverage_report(step):
    world.assert_no_file(EXPECTED_COVERAGE_XML)
    world.assert_no_file(EXPECTED_COVERAGE_HTML)
